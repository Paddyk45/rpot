...
impl Packet {
    ...
    fn parse_payload(buffer: Vec<u8>) -> String {
...
impl Packet {
    pub fn to_u8_arr(&self) -> Vec<u8> {
        ...
        if self.length.is_none() {
            ...
            length += self.payload.len() as i32 + 1; // Payload length + NULL-terminator
        ...
        let payload_buf = self.payload.as_bytes();
        buffer.extend_from_slice(payload_buf);
